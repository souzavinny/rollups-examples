...
This will create a file at `./deployments/<network>/biometrics.json` with the deployed contract's address.
...
```shell
yarn start input send --payload "Hello there" --addressFile path/to/biometrics/deployments/<network>/biometrics.json
```
