...
def send_chunks(content, input):
    for idx, x in enumerate(content):
        if((idx+1)==len(content)):
            ...
            if(idx == 0):
                strinput = {"chunk": "initial", "content": x,"imageId": input}
            else:
                strinput = {"chunk": idx+1, "content": x,"imageId": input}
        inp = json.dumps(strinput)
        call_docker(inp)
...
def call_docker(h_input):
    subprocess.call(f"yarn start input send --payload \'{h_input}\' --address 0x70ac08179605AF2D9e75782b8DEcDD3c22aA4D0C", shell=True)
